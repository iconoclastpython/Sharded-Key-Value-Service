...
const (
	Decided   Fate = iota + 1
	Pending   Fate = iota     	  // not yet decided.
...
type PaxosArg struct {
	...
	PNum string
	PValue interface{}
	Done int // done value for the sender side, use when send decision
	Me int // identification of sender, index from the peer[] array
}

type PaxisReply struct {
	Result int
	PNum string
	PValue interface{}
}
...
func (px *Paxos) GeneratePaxosNumber() string {
	begin := time.Date(2016, time.April, 3, 40, 0, 0, 0, time.EST)
...
func (px *Paxos) MakePaxosInstance(id int, v interface{}) {
	if _, exist := px.instances[id] = &PaxosInstance{
		client: v,
		pid: id, 
		decided: false
		prepared: Pending
		accepted: Proposal{PNum: Pending}
	}
}
...
func (px *Paxos) ProcessAccept(arg *PaxosArg, reply * PaxosReply) error {
	...
	pnum := arg.PNum
	pvalue := arg.PValue
	reply.Result = Forgotten
	_, exist := px.instances[pid]
	if !exist {
		px.MakePaxosInstance(pid, nil)
	}
	if pnum >= px.instances[pid].prepared {
		px.instance[pid].accepted = Proposal{pnum, pvalue}
		px.instance[pid].prepared = pnum
		reply.Result = Decided
	}
	return nil
} 
...
func (px *Paxos) SendPrepare(pid int, v interface{}) (bool, []string, Proposal) {
	...
	for i, acceptors := range acceptors {
		arg := PaxosArg{Pid: pid, PNum: pnum, Me: px.me}
		var reply PaxosReply = PaxosReply{Result: Forgotten}
		if i == px.me {
			px.ProcessPrepare(&arg, &reply)
		}else{
			call(acceptors, "Paxos.ProcessPrepare", &arg, &reply)
		}
		if reply.Result == Decided {
...
func (px *Paxos) SendAccept(pid int, acceptors []string, proposal Proposal) bool {
	...
	for i, peer := range acceptors {
		...
		var reply PaxosReply = PaxosReply{Reply: Forgotten}
		if i == px.me {
			px.ProcessAccept(&arg, &reply)
		}else{
			call(peer, "Paxis.ProcessAccept", &arg, &reply)
		}
		if reply.Result == Decided {
			num++
		}
...
func (px *Paxos) MakeDecision(pid int, result Proposal) {
	if _, exist := px.instances[pid]; !exist{
		px.MakePaxosInstance(pid, nil)
	}
	px.instances[pid].accepted = proposal
	px.instance[pid].decided = true
}
...
func (px *Paxos) Min() int {
	...
	for m, _ := range px.instances {
		if m <= min && px.instances[m].decided {
			delete(px.instances, m)
		}
	}
